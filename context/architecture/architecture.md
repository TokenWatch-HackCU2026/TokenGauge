...
TokenGauge is an AI usage intelligence platform. Users wrap their existing AI provider clients with the TokenGauge SDK. The SDK captures usage data (tokens, cost, latency) from each API response and forwards it to TokenGauge in the background. TokenGauge never acts as a network proxy and never receives or stores provider API keys.

```
User's App
    │
    ▼
TokenGauge SDK (wraps provider client)
    │                    │
    ▼                    ▼
AI Provider          POST /usage/  →  TokenGauge API  →  MongoDB
...
### 1. TokenGauge SDK (Python + JavaScript)
- User installs `pip install tokengauge` or `npm install tokengauge`
- User initializes with their long-lived TokenGauge SDK token
- SDK wraps the user's existing provider client (OpenAI, Anthropic, Google, Mistral)
- All API calls go **directly** from the user's app to the provider — TokenGauge is never in the network path
...
- SDK fires a background POST to `POST /usage/` on TokenGauge (non-blocking, does not delay the user's response)
- If TokenGauge is unreachable, SDK queues the event and retries — user's app is never affected
...
tw = TokenGauge(api_key="tw-abc123...")     # long-lived SDK token
...
# SDK captures usage from response.usage and POSTs to TokenGauge
```
...
const tw = new TokenGauge({ apiKey: 'tw-abc123...' })
...
### 2. Auth System
- JWT-based authentication for the TokenGauge web app (login, dashboard)
...
- SDK event queue: buffer for retry when TokenGauge API is temporarily unreachable
...
3. Call goes directly to provider (e.g. `https://api.openai.com`) — TokenGauge not in path
...
7. SDK spawns background thread/task: `POST /usage/` to TokenGauge with SDK token auth
8. SDK returns original provider response to user immediately — step 7 is non-blocking
9. TokenGauge API validates SDK token, inserts record into `api_calls`

---

## Security Model

- **Provider API keys**: TokenGauge never receives, stores, or sees them — they stay entirely in the user's environment
...
- **TLS**: All TokenGauge API traffic over TLS 1.3
...
- Sent alongside usage data to TokenGauge
...
- Source: `tokenwatch-sdk/` directory in this repo (published as `tokengauge` on PyPI)
